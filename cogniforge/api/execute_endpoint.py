...
class ExecutionRequest(BaseModel):
    ...
    object_name: Optional[str] = Field(
        None,
        description="Primary object to manipulate",
        example="blue_cube"
    )
    ...
    use_vision: bool = Field(
        True,
        description="Whether to use vision for refinement"
    )
    
    use_gpt_reward: bool = Field(
        False,
        description="Whether to use GPT for reward computation"
    )
    
    num_bc_epochs: int = Field(
        10,
        description="Number of behavior cloning epochs",
        ge=1,
        le=100
    )
    ...
    num_optimization_steps: int = Field(
        50,
        description="Number of policy optimization steps",
        ge=10,
        le=500
    )
    ...
    safety_checks: bool = Field(
        True,
        description="Enable safety checks during execution"
    )
    
    dry_run: bool = Field(
        False,
        description="Simulate execution without robot movement"
    )
...
class PipelineOrchestrator:
        ...
        try:
            ...
            self.update_status(
                ...
                metrics={
                    ...
                    "avg_trajectory_length": np.mean([len(t.get("states", [])) for t in trajectories])
                }
            )
            ...
            code = await self._generate_execution_code(
                plan,
                request,
                vision_result if request.use_vision else None,
                request_id
            )
            ...
            if not request.dry_run:
                ...
                exec_result = await self._execute_on_robot(
                    code,
    ...
    async def _execute_expert_demo(
        ...
        for i in range(num_demos):  # Collect 5 demonstrations
            ...
            traj = self.expert.collect_demonstration(
    ...
    async def _execute_vision_refinement(
        ...
        self._emit_heartbeat(
            ...
            metrics={
                "pixel_offset_x": dx_px,
                "pixel_offset_y": dy_px
            }
        )
        await asyncio.sleep(0.05)
        ...
        self._emit_heartbeat(
            ...
            metrics={
                "aligned": vision_result["vision_detection"].get("alignment", {}).get("is_aligned", False),
                "confidence": 0.85
            }
        )
...
async def create_demo_event_stream():
    ...
    async def demo_pipeline():
        ...
        stages = [
            ...
            (PipelineStage.PLANNING, 0.15, "Task plan generated", {"steps": 5, "planning_time": 1.2}),
            (PipelineStage.EXPERT_DEMO, 0.2, "Collecting expert demonstrations", {"target_demos": 5}),
            (PipelineStage.EXPERT_DEMO, 0.3, "Expert demonstrations collected", {"num_trajectories": 5, "avg_length": 100}),
            (PipelineStage.BEHAVIOR_CLONING, 0.35, "Starting BC training", {"epochs": 10, "batch_size": 32}),
            (PipelineStage.BEHAVIOR_CLONING, 0.5, "BC training complete", {"bc_loss": 0.0234, "training_time": 3.5}),
            (PipelineStage.OPTIMIZATION, 0.55, "Starting policy optimization", {"algorithm": "PPO", "steps": 50}),
            (PipelineStage.OPTIMIZATION, 0.7, "Optimization complete", {"final_reward": 0.875, "optimization_time": 5.2}),
            (PipelineStage.VISION_REFINEMENT, 0.75, "Capturing vision data", {"camera": "wrist_camera"}),
            (PipelineStage.VISION_REFINEMENT, 0.85, "Vision refinement complete", {"aligned": True, "offset_mm": 2.3, "confidence": 0.92}),
            (PipelineStage.CODE_GENERATION, 0.88, "Generating execution code", {"language": "python", "lines": 150}),
            (PipelineStage.CODE_GENERATION, 0.92, "Code generation complete", {"functions": 8, "safety_checks": True}),
            (PipelineStage.EXECUTION, 0.95, "Executing on robot", {"robot_model": "UR5", "gripper": "Robotiq"}),
            (PipelineStage.COMPLETED, 1.0, "Task completed successfully", {"total_time": 25.3, "success": True})
        ]
...
async def download_code(code_id: str):
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f'attachment; filename="{code_id}.py"'
        }
    )
...
async def get_execution_log(request_id: str):
    ...
    events = []
    if hasattr(orchestrator.event_stream, 'events') and request_id in orchestrator.event_stream.events:
...
async def get_execution_summary(request_id: str):
    ...
    summary = {
        ...
        "code": {
            ...
            "preview": result.generated_code[:500] + "..." if result.generated_code and len(result.generated_code) > 500 else result.generated_code
        },
        ...
        "metrics": {
            ...
            "vision_metrics": {
                ...
                "aligned": result.vision_alignment.get("alignment", {}).get("is_aligned") if result.vision_alignment else None,
                "offset_mm": result.vision_alignment.get("alignment", {}).get("error_mm") if result.vision_alignment else None,
                "confidence": result.vision_alignment.get("confidence") if result.vision_alignment else None
            },
        ...
        # Quick actions
        "actions": {
            "view_code": f"http://localhost:8000/code/preview/{result.code_links.code_id}" if result.code_links else None,
            "download_code": f"http://localhost:8000/code/download/{result.code_links.code_id}" if result.code_links else None,
